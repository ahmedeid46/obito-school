...
class Onlineexam extends Student_Controller {
    ...
    public function getExamForm() {
        ...
        if (strtotime(date('Y-m-d H:i:s')) >= strtotime(date($exam->duration.$exam->exam_to ))) {
            $question_status = 1;
            $data['question_status'] = 1;

        }elseif ((strtotime(date('Y-m-d H:i:s')) <= strtotime(date($exam->duration.$exam->exam_from )))){
            $question_status = 1;
            $data['question_status'] = 1;
        ...
        $time=strtotime(date($exam->duration.$exam->exam_to)) - strtotime(date('Y-m-d H:i:s'));
        $data['time']=$time;
        $questionOpt = $this->customlib->getQuesOption();
        $data['questionOpt'] = $questionOpt;
        $pag_content = $this->load->view('user/onlineexam/_searchQuestionByExamID', $data, true);
        echo json_encode(array('status' => 0,'time'=>$time, 'exam' => $exam, 'page' => $pag_content, 'question_status' => $question_status));
    }
