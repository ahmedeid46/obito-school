...
class Chat extends Admin_Controller {
    ...
    public function upload_file(){
        if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
            ...
            $img_name = rand().rand(). '.' . $fileInfo['extension'];
            move_uploaded_file($_FILES["file"]["tmp_name"], "./uploads/chat/" . $img_name);
            ...
            $insert_record = array(
                ...
                'files'=>'uploads/chat/' . $img_name,
                'chat_connection_id' => $chat_connection_id,
                'created_at' => date('Y-m-d H:i:s', $this->customlib->dateTimeformatTwentyfourhour($time, true)),
            );

            $this->last_insert_id = $this->chatuser_model->addMessage($this->security->xss_clean($insert_record));

            $array = array('status' => '1', 'last_insert_id' => $this->last_insert_id, 'error' => '', 'message' => $this->lang->line('inserted'));
            echo json_encode($array);
        }else{
            $array = array('status' => '0', 'last_insert_id' => "error", 'error' => "can't find this file", 'message' => "not ".$this->lang->line('inserted'));
            echo json_encode($array);
        }
    }
    function downloadFileChat($file){
        $this->load->helper('download');
        $name = $this->uri->segment(5);
        $filepath = "./uploads/chat/" . $file;
        $data = file_get_contents($filepath);
        force_download($name, $data);
    }

    public function upload_audio(){
        if (isset($_FILES["audio_data"]) && !empty($_FILES['audio_data']['name'])) {
            $fileInfo = pathinfo($_FILES["audio_data"]["name"]);
            $img_name = rand().rand(). '.' . "wav";
            $audio=$_FILES["audio_data"]["tmp_name"];
            if(move_uploaded_file($audio, "./uploads/chat/audio/" . $img_name)) {
                $chat_connection_id = $this->input->post('chat_connection_id');
                $chat_to_user = $this->input->post('chat_to_user');
                $time = $this->input->post('time');
                $insert_record = array(
                    'chat_user_id' => $chat_to_user,
                    'audio' => 'uploads/chat/audio/' . $img_name,
                    'chat_connection_id' => $chat_connection_id,
                    'created_at' => date('Y-m-d H:i:s', $this->customlib->dateTimeformatTwentyfourhour($time, true)),
                );
                ...
                $array = array('status' => '0',  'error' => 'file not upload', 'message' =>"not ".$this->lang->line('inserted'));
                echo json_encode($array);
            }
        }else{
            $array = array('status' => '0', 'last_insert_id' => "error", 'error' => "can't find this file", 'message' => "not ".$this->lang->line('inserted'));
