...
class Student extends Admin_Controller
{
    ...
    public function changepass($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            //if ($query1) {
                $query2 = $this->user_model->saveNewPassbyid($newdata);
                if ($query2 == true) {
    ...
    public function changeusername($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!$is_exists) {
                $is_updated = $this->user_model->saveNewUsernamebyid($newdata);
                if ($is_updated == true) {
                    $this->session->set_flashdata('success_msg', 'Username changed successfully');
                    redirect('student/changeusername/'.$id);
                }else{
                    $this->session->set_flashdata('error_msg', 'Username changed error');
                    redirect('student/changeusername/'.$id);
                }
            } else {
        ...
        $this->data['id']       = $id;
        $this->load->view('layout/header', $data);
        $this->load->view('student/change_username', $data);
        $this->load->view('layout/footer', $data);
    }
